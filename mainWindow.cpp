...
mainWindow::mainWindow(QMainWindow *parent) : QMainWindow(parent), mainWindow_ui(new Ui::MainWindow)
{
    ...
    //connect(imageCapture, SIGNAL(processImage(cv::Mat &)), this, SLOT(displayImage(cv::Mat &)));
...
void mainWindow::displayImage(cv::Mat &image) {
    ...
    //image = cv::cvMatToQImage(image);
    mainWindow_ui->displayLabel->setPixmap(QPixmap::fromImage(imageConvert(image)));
    mainWindow_ui->displayLabel->adjustSize();
}

QImage mainWindow::imageConvert(cv::Mat &matImage) {
